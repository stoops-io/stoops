...
|------|:---:|:---:|:---:|:---:|:---:|
| **Admin** | yes | yes | yes | yes | admin, participant, observer |
| **Participant** | yes | yes | no | no | participant, observer |
| **Observer** | no | no | no | no | none |

Authority is set on join via the share token's tier and does not change for the session.

**Observers** are invisible: they don't appear in participant lists, don't emit `ParticipantJoined`/`ParticipantLeft` events, and have no input field in the TUI. They connect via `Room.observe()` internally.
...
Generated on server startup (one admin, one participant) and via `POST /share` or `/share` slash command. Callers can only generate tokens at their own tier or below — enforced by `canGrant()` which compares tier ordering (admin: 2, participant: 1, observer: 0).

```
https://server:7890?token=a3f8c1...  → admin
https://server:7890?token=7b2e9d...  → participant
```
...
- `POST /message` — 403 if observer
- `POST /event` — 403 if observer
- `POST /set-mode` — setting own mode: any non-observer. Setting another's mode: admin only
- `POST /kick` — admin only, 403 otherwise
- `POST /share` — observer blocked. Participant generates at own tier or below. Admin generates at any tier
...
This is a UX convenience — the server enforces regardless. An observer agent still has `stoops__send_message` visible but gets 403 if it tries to use it.

---

## TUI slash commands by tier

| Command | Admin | Participant | Observer |
|---------|:---:|:---:|:---:|
| `/who` | yes | yes | yes |
| `/leave` | yes | yes | yes |
| `/kick <name>` | yes | no | no |
...
2. Participant token — printed as `Join: stoops join <url>` and `Agent: stoops run claude --join <url>`

### Via `/share` slash command

Admin calling `/share` generates links at all tiers they can grant:
```
/share              → admin + participant + observer links
/share --as observer → observer link only
```

Participant calling `/share` generates links at their tier and below:
```
/share              → participant + observer links
...
**Authority is structural, engagement is behavioral.** An admin in standby has full power but isn't listening. A participant in everyone mode has limited tools but is fully engaged. The two axes are independent.
...
**Permissions cascade naturally.** A participant generates a participant share link and runs `stoops run claude --join <that-link>`. Their agent joins at participant tier. No admin approval needed. Admin has `/kick` for cleanup.
